...
//         when { branch "master" }
    ...
    stage('MNIST Predictor - deploy'){
        ...
        steps {
            ...
            # replace registry url and image name placeholders in yaml
             sed -e  "s/{{REGISTRY_URL}}/$REGISTRY_URL/g" mnist-predictor.yaml
             sed -e "s/{{K8S_NAMESPACE}}/$K8S_NAMESPACE/g" mnist-predictor.yaml
             sed -e "s/{{IMG_NAME}}/$IMG_NAME/g" mnist-predictor.yaml
